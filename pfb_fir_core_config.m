...
function pfb_fir_core_config(this_block)
  ...
  % Revision History:
  %
  %   23-Jan-2014  (23:14 hours):
  %     Original code was machine generated by Xilinx's System Generator after parsing
  %     /tools/designs/CASPER/projects/leda/davidm/adc16/svi/pfb_fir_core.vhd
  %
  %
  ...
  out0_port.setType('UFix_40_0');
  out1_port = this_block.port('out1');
  out1_port.setType('UFix_40_0');
  out2_port = this_block.port('out2');
  out2_port.setType('UFix_40_0');
  ...
  if (this_block.inputTypesKnown)
    ...
    if (this_block.port('in0').width ~= 32);
      this_block.setError('Input data type for port "in0" must have width=32.');
    end

    if (this_block.port('in1').width ~= 32);
      this_block.setError('Input data type for port "in1" must have width=32.');
    end

    if (this_block.port('in2').width ~= 32);
      this_block.setError('Input data type for port "in2" must have width=32.');
    end

    if (this_block.port('in3').width ~= 32);
      this_block.setError('Input data type for port "in3" must have width=32.');
    end
